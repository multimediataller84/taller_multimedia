    ...
    >
      <div
        ...
        <div className="flex flex-col overflow-y-auto h-[30%] space-y-2">
          {loading ? (
            <p className="text-gray1 text-center">Cargando perfiles...</p>
          ) : filteredProfiles.length === 0 ? (
            <p className="text-gray1 text-center">No se encontraron perfiles</p>
          ) : (
            filteredProfiles.map((profile) => (
              <div
                key={profile.id}
                className={`cursor-pointer rounded-xl px-4 py-3 shadow transition
                ${
