...
useEffect(() => {
  ...
  return (
    <div className="flex flex-col absolute justify-center items-center size-full bg-gradient-to-r from-backgroundBlue to-white "> 
    
    <div
        ref={$follower}
        className="pointer-events-none fixed left-0 top-0 aspect-square w-[20vmin] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-full will-change-transform z-0 "
        >
          <div className="h-full w-full animate-spin-slow bg-gradient-to-r from-blue-200 to-blue-300 blur-3xl"></div>
      </div>
      
      <div className="fixed top-5 left-5 size-20 bg-blue-500"></div>
      <div className="fixed top-100 right-98 size-20 bg-blue-500"></div>
      <div className="fixed bottom-80 left-98 size-20 bg-blue-500"></div>
      ...
      <div className="relative w-[284px] flex">
        <input
          ...
          className={` peer border w-full h-[48px] rounded-3xl px-4 pr-12 bg-white 
            ${
      ...
      {errors.password?.message && (
        <p className="text-red-500 text-sm font-lato text-center">{errors.password.message}</p>
      )}
        ...
        {loading ? (
          ...
          <button
            type="submit"
            className="translate-y-10 w-full h-[48px] hover:bg-blue-500 bg-black text-white font-semibold py-2 rounded-3xl transition duration-300 delay-75"
          >
            Iniciar Sesión
          </button>
        )}

        {error && <p className="translate-y-15 text-red-500 text-sm font-lato text-center ">{error}</p>}
        </form>
      </div>
